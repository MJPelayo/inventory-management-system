    ...
    try {
        ...
        const result = await pool.query(
            `SELECT
                ...
                COALESCE(SUM(i.quantity), 0) AS current_occupancy
             FROM warehouses w
...
const createWarehouse = async (req, res) => {
    try {
        ...
        await createAuditLog({
            ...
            new_values: { name: result.rows[0].name, location: result.rows[0].location, capacity: result.rows[0].capacity },
            ip_address: req.ip,
            user_agent: req.get('user-agent'),
            notes: 'Warehouse created'
        });
...
const updateWarehouse = async (req, res) => {
    try {
        ...
        const user = req.user;
        const changes = {};
        if (name !== undefined && name.trim() !== current.name) changes.name = { old: current.name, new: name.trim() };
        if (location !== undefined && location !== current.location) changes.location = { old: current.location, new: location };
        if (capacity !== undefined && newCapacity !== current.capacity) changes.capacity = { old: current.capacity, new: newCapacity };
        if (current_occupancy !== undefined && newOccupancy !== current.current_occupancy) changes.current_occupancy = { old: current.current_occupancy, new: newOccupancy };
        if (is_active !== undefined && is_active !== current.is_active) changes.is_active = { old: current.is_active, new: is_active };
        ...
        await createAuditLog({
            ...
            entity_name: result.rows[0].name,
            old_values: { name: current.name, location: current.location, capacity: current.capacity, is_active: current.is_active },
            new_values: { name: result.rows[0].name, location: result.rows[0].location, capacity: result.rows[0].capacity, is_active: result.rows[0].is_active, changes },
            ip_address: req.ip,
            user_agent: req.get('user-agent'),
            notes: 'Warehouse updated'
        });
...
const deleteWarehouse = async (req, res) => {
    try {
        ...
        await createAuditLog({
            ...
            old_values: { id: existing.rows[0].id, name: existing.rows[0].name },
            ip_address: req.ip,
            user_agent: req.get('user-agent'),
            notes: 'Warehouse deleted'
        });
