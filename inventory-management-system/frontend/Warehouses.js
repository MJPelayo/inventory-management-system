// frontend/Warehouses.js - Updated with authentication support
...
function renderCards(filtered) {
    ...
    // Check permissions
    const canEdit = Auth.hasPermission('warehouse:update') || Auth.isAdmin();
    const canDelete = Auth.hasPermission('warehouse:delete') || Auth.isAdmin();
    ...
    grid.innerHTML = filtered.map(w => {
        ...
        if (canEdit || canDelete) {
            ...
            actionsHtml += `<button class="btn btn-sm" onclick="openDetail(${w.id})">View inventory</button>`;
            if (canEdit) {
                actionsHtml += `<button class="btn btn-sm" onclick="editWarehouse(${w.id})">Edit</button>`;
            }
            if (canDelete) {
                actionsHtml += `<button class="btn btn-sm btn-danger" onclick="deleteWarehouse(${w.id}, '${escHtml(w.name)}')">Delete</button>`;
            }
            actionsHtml += `</div>`;
        } else {
            actionsHtml = `<div class="card-actions"><button class="btn btn-sm" onclick="openDetail(${w.id})">View inventory</button></div>`;
        }
...
function openModal(id = null) {
    // Check permission
    if (editingId !== null && !Auth.hasPermission('warehouse:update') && !Auth.isAdmin()) {
        alert('You do not have permission to edit warehouses.');
        return;
    }
    if (editingId === null && !Auth.hasPermission('warehouse:create') && !Auth.isAdmin()) {
...
async function saveWarehouse() {
    // Check permissions
    if (editingId !== null && !Auth.hasPermission('warehouse:update') && !Auth.isAdmin()) {
        return showFormError('You do not have permission to edit warehouses.');
    }
    if (editingId === null && !Auth.hasPermission('warehouse:create') && !Auth.isAdmin()) {
        return showFormError('You do not have permission to create warehouses.');
    }
...
async function deleteWarehouse(id, name) {
    // Check permission
    if (!Auth.hasPermission('warehouse:delete') && !Auth.isAdmin()) {
...
// ─── Init ─────────────────────────────────────────────────────────────────────
// Authentication check is now handled in Warehouse.html
...
if (document.readyState === 'loading') {
    ...
    // DOM already ready, load warehouses
    loadWarehouses();
}