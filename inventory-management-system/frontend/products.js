...
function renderTable(filtered) {
    ...
    const canEdit = Auth.hasPermission('product:update');
    const canDelete = Auth.hasPermission('product:delete');
    ...
    tbody.innerHTML = page.map(p => {
        ...
        let actionsHtml = '<span style="color:var(--text-3)">View only</span>';
        if (canEdit || canDelete) {
            actionsHtml = '<div class="actions-cell">';
            if (canEdit) {
                actionsHtml += `<button class="btn btn-sm" onclick="editProduct(${p.id})">Edit</button>`;
            }
            if (canDelete) {
                actionsHtml += `<button class="btn btn-sm btn-danger" onclick="deleteProduct(${p.id}, '${escHtml(p.name)}')">Delete</button>`;
            }
            actionsHtml += '</div>';
        }
...
function openModal(id = null) {
    // Check permission for create/update
    if (editingId !== null && !Auth.hasPermission('product:update')) {
        alert('You do not have permission to edit products.');
        return;
    }
    if (editingId === null && !Auth.hasPermission('product:create')) {
...
async function saveProduct() {
    // Check permissions
    if (editingId !== null && !Auth.hasPermission('product:update')) {
        return showError('You do not have permission to edit products.');
    }
    if (editingId === null && !Auth.hasPermission('product:create')) {
        return showError('You do not have permission to create products.');
    }
...
// ─── Init ─────────────────────────────────────────────────────────────────────
// Authentication check is now handled in Products.html